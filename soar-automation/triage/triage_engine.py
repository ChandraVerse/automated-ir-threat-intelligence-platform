"""
Alert Triage Engine
Prioritises incoming Wazuh alerts using rule level, MITRE tactic, and IOC verdict.
"""
...
# MITRE tactics that bump priority
HIGH_IMPACT_TACTICS = {
...
class TriageEngine:
    """
    Scores alerts based on:
      - Wazuh rule level (0-15)  → weight 40%
      - IOC verdict score        → weight 45%
      - MITRE tactic             → weight 15%
    """
    ...
    def triage(self, alert: dict[str, Any], ioc_score: float = 0.0) -> TriageResult:
        ...
        mitre_tactics: list[str] = rule.get("mitre", {}).get("tactic_id", [])

        # Normalise rule level (0-15) → 0-100
    ...
    def _recommend_playbook(alert: dict, ioc_score: float, priority: Priority) -> str | None:
        ...
        if verdict == "MALICIOUS" and ioc_type == "ip":
            return "malicious_ip"
        if verdict == "SUSPICIOUS" and ioc_type == "ip":
            return "suspicious_ip"
        if verdict == "MALICIOUS" and ioc_type == "hash":
